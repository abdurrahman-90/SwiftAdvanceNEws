...
class Glide : NSObject, UIGestureRecognizerDelegate, UIScrollViewDelegate {
    ...
    private func addChildToContainer(containerView: UIView, card: UIView) {
        guard let safeAreaLayout = UIApplication.shared.windows.filter({$0.isKeyWindow}).first?.safeAreaLayoutGuide.layoutFrame.height else { return }
        guard let bottomAreaInset = UIApplication.shared.windows.filter({$0.isKeyWindow}).first?.safeAreaInsets.bottom else { return }
    ...
    private func setupRecognizer(card : UIView) {
        guard let _ = self.card else { return }
    ...
    @objc func handlePanRecognizer(recognizer: UIPanGestureRecognizer) {
        guard let safeAreaLayout = UIApplication.shared.windows.filter({$0.isKeyWindow}).first?.safeAreaLayoutGuide.layoutFrame.height else { return }
        guard (UIApplication.shared.windows.filter({$0.isKeyWindow}).first?.safeAreaInsets.bottom) != nil else { return }
    ...
    private func dimAlphaWithCardTopConstraint(value: CGFloat) -> CGFloat {
      let fullDimAlpha : CGFloat = 0.4
        ...
        guard let safeAreaHeight = UIApplication.shared.windows.filter({$0.isKeyWindow}).first?.safeAreaLayoutGuide.layoutFrame.size.height,
              let bottomPadding = UIApplication.shared.windows.filter({$0.isKeyWindow}).first?.safeAreaInsets.bottom else {
        return fullDimAlpha
      }
